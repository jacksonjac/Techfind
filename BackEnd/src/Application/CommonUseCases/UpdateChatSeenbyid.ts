...
export const UpdateChatseenbyIds = (dependencies: any) => {
  
  const { UpdatechatSeenRepo } = dependencies.repositery;

  const executeFunction = async (data:any) => {
    try {
      // Fetch user data
      const responseFromUserList = await UpdatechatSeenRepo.PostExit(data);
